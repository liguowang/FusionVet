...
BAM OP  Description
0   M   alignment match	(move ref coordinates)
1   I   insertion to the reference
2   D   deletion from the reference	(move ref coordinates)
3   N   skipped region from the reference	(move ref coordinates)
...
7   =   sequence match	(move ref coordinates)
8   X   sequence mismatch	(move ref coordinates)
9	B	BAM_CBACK

sum of M, I, S, =, X  shall equal SEQ length
Example:
...
def map_bounds(start_pos, cigar):
    ...
    for c,s in cigar:   #code and size
        if c == 0:  	#alignment match (ref coordinates move)
            span_size += s
        elif c == 2: 	#deletion to the ref (ref coordinates move)
            span_size += s
        elif c == 3:    # skipping to the ref (ref coordinates move)
            span_size += s
        elif c == 7: 	#sequence match (ref coordinatesn move)
        	span_size += s
        elif c == 8:	#sequence mismatch (ref coordinatesn move)
        	span_size += s
        else:
            continue
